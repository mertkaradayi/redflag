// LLM Contract Analyzer - Integrated into RedFlag Backend
// Based on 4-Agent + 1-Critic Chain Architecture
 ...
 */
async function callGemini(prompt: string, isJson = false) {
    ...
    try {
        ...
        try {
            ...
            if (isJson) {
                ...
                try {
                    ...
                    try {
                        ...
                        console.log('[Gemini Call] Successfully parsed JSON after fixing');
                        return parsed;
                    } catch (fixError) {
                        console.error('[Gemini Call] FAILED TO PARSE JSON even after fixing.');
                        ...
                        console.warn('[Gemini Call] Using fallback response due to JSON parsing failure');
                        return {
            ...
            console.error('[Gemini Call] Error processing response:', (e as Error).message);
            console.error('The AI returned this text:');
            console.error('----------------- AI RESPONSE START -----------------');
        ...
        console.error('[Gemini Call] Error:', (error as Error).message);
        throw error;
    }
}
 ...
 */
function validateAndFinalize(safetyCard: any) {
 ...
 */
function buildTriagePrompt(publicFunctions: any[], structDefinitions: any, dependencyRisks: any[]) {
    return `
...
/**
 * Agent 2: Technical Analyst - Deep analysis of flagged functions
 */
function buildTechAnalysisPrompt(publicFunctions: any[], riskyFunctionNames: string[], structDefinitions: any, dependencyRisks: any[], disassembledCode: any) {
    return `
...
- Explain the ROOT CAUSE using struct field details
...
- Extract the specific code snippet that demonstrates the vulnerability
...
/**
 * Builds the prompt for Agent 3: Risk Scorer
 */
function buildScoringPrompt(technicalFindings: any[]) {
 ...
 */
function buildReportPrompt(technicalFindings: any[], riskScoreReport: any, dependencyRisks: any[]) {
    return `
 ...
 */
function buildCriticPrompt(technicalFindings: any[], finalReportDraft: any) {
    return `
 ...
 */
function buildCorrectionPrompt(finalReportDraft: any, criticFeedback: string) {
    return `
...
export async function runFullAnalysisChain(packageId: string, network: string, suiClient: SuiClient, cacheKey: string) {
    ...
    for (const structId of uniqueStructs) {
        try {
            ...
            console.log(`[CHAIN 2.5/5] Warning: Could not fetch struct ${structId}: ${(error as Error).message}`);
    ...
    if (triageResult.fallback) {
        console.warn('[CHAIN 1/5] Triage failed due to JSON parsing error, using fallback');
        const safeCard = {
            summary: "Analysis incomplete due to AI response parsing error. Manual review recommended.",
            risky_functions: [],
            rug_pull_indicators: [],
            ...
            risk_score: 50, // Medium risk due to incomplete analysis
    ...
    // Handle fallback response
    if (techResult.fallback) {
        console.warn('[CHAIN 2/5] Technical Analysis failed due to JSON parsing error, using fallback');
        const safeCard = {
            summary: "Analysis incomplete due to AI response parsing error. Manual review recommended.",
            risky_functions: [],
            rug_pull_indicators: [],
 ...
 */
export function getCachedAnalysis(packageId: string, network: string) {
