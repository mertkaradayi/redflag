...
export function EvidenceBlock({ text, className }: EvidenceBlockProps) {
  ...
  return (
    <div className={cn('w-full min-w-0 max-w-full overflow-x-hidden', className)}>
      {/* Explanation text */}
      {explanation && (
        <p className="text-sm leading-relaxed text-foreground/80 dark:text-white/80 mb-2 break-words">
          {explanation}
        </p>
      )}
      ...
      {codeSnippets.length === 0 && !explanation && (
        <p className="text-sm leading-relaxed text-foreground/80 dark:text-white/80 break-words">
          {text}
        </p>
      )}
    </div>
